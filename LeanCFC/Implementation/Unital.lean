...
calculus with `R := ℝ≥0`. From this we can define `√a := cfc NNReal.sqrt a`, which is also
...
+ for `a : A`, the `R`-spectrum of `a` is compact.
+ for `a : A` satisfying `p a`, the `R`-spectrum of `a` is nonempty.
...
/-- `cfcHom (ha : p a)` sends the (restriction onto the spectrum of the) identity function
to `a` itself. -/
lemma cfcHom_id :
...
/-- Any continuous ⋆-homomorphism `φ : C(spectrum R a, R) →⋆ₐ[R] A` with `φ a = a`
must be `cfcHom (ha : p a)`. -/
...
/-- A lemma stating the composition property for `cfcHom`. Because of type
considerations, we must introduce an auxiliary function `f'` whose codomain is the
spectrum of `cfcHom ha f`, but whose function values (coerced to `R`) match that
of `f`. From a mathematician's perspective, the only difference between `f` and
`f'` is that `f` has an expanded codomain.

The difficulty in stating this lemma is one of the primary reasons to prefer an
unbundled continuous functional calculus `cfc`. -/
theorem cfcHom_comp [UniqueHom R A] (f : C(spectrum R a, R))
  ...
  /- Define a ⋆-homomorphism from the continuous functions on the spectrum of
  `cfcHom ha f` which is given by composing `cfcHom ha` (the continuous
  functional calculus on `a`) with the natural ⋆-homomorphism
  `C(spectrum R (cfcHom ha f), spectrum R a)` given by precomposition with `f'`.
  In other words, the map `g ↦ cfcHom ha (g.comp f')` is a ⋆-homomorphism. -/
  let φ : C(spectrum R (cfcHom ha f), R) →⋆ₐ[R] A :=
    (cfcHom ha).comp <| ContinuousMap.compStarAlgHom' R R f'
  /- By uniqueness of the continuous functional calculus, it suffices to show that
  this map `φ` is continuous and sends the (restriction of the identity) to
  `cfcHom ha f`. -/
...
-- This version is useful as ⋆-algebra homomorphisms do not have a proof of
-- continuity bundled into their type.
...
/-- expands `cfc f a` to its definition, in the case of non-junk values -/
...
/-- expands `cfc f a` to its definition, in the junk-value case -/
...
/-- applying `cfcHom` to `f` (which is necessarily continuous on the spectrum), is the same
as applying `cfc` to *any* extension of `f` to all of `R`, continuous or otherwise. -/
...
/-- We often prove basic facts about `cfc` by splitting into cases according to whether
we are in the junk value situation or not. This theorem simply makes such arguments slightly
more convenient.

Unfortunately, the motive `P` generally needs to be specified explicitly. -/
...
lemma cfc_commute_cfc (f g : R → R) (a : A) : Commute (cfc f a) (cfc g a) := by
  /- by using `cfc_cases`, we can take advantage of the junk values to avoid adding hypotheses
  on the functions `f, g` or on the element `a`. -/
...
variable (R) in
include R in
/-- The reason the `predicate_zero` field is included in the
`ContinuousFunctionalCalculus` structure is that we want to always conclude that the
scalars in the algebra satisfy the relevant predicate for the functional calculus. -/
...
/-- A key tool in arguments involving the continuous functional calculus.
The functional calculus applied to two different functions `f` and `g` are
equal so long as these functions are equal on the spectrum. -/
lemma cfc_congr {f g : R → R} {a : A} (hfg : (spectrum R a).EqOn f g) :
    cfc f a = cfc g a := by
  /- by taking advantage of junk values, we again avoid the need for extraneous
  hypotheses on `f`, `g` and `a`. -/
...
/-- The converse of the previous lemma. In this direction, the extra hypotheses are
necessary. -/
...
/- The following collection of lemmas are unbundled versions of the fact that `cfcHom`
is a ⋆-homomorphism. Because of junk values, `cfc` is *not* a ⋆-homomorphism, so we
need specialized lemmas with the hypothesis that `a` satsifies the predicate `p`. -/
...
/- The continuous functional calculus extends the polynomial functional calculus (also
known as *evaluation*). -/
...
/-- The *composition property* of the continuous functional calculus. Note that, while
this makes essential use of `cfcHom_comp` for the proof, this version has natural
assumptions on `f`, `g` and `a`, the same as a mathematician would expect. There is no
need for an auxiliary function `f'` because of the unbundling. -/
...
/-- A version of `cfc_comp` with `g ∘ f` expanded to `fun x ↦ g (f x)`, as these are not
reducibly definitionally equal in Lean 4 (they were in Lean 3). -/
lemma cfc_comp' (g f : R → R) (a : A) (hg : ContinuousOn g (f '' spectrum R a) := by cfc_cont_tac)
    (hf : ContinuousOn f (spectrum R a) := by cfc_cont_tac) (ha : p a := by cfc_tac) :
    cfc (g <| f ·) a = cfc g (cfc f a) :=
  cfc_comp g f a

/-- `cfc_comp` specialized to the function `f := fun x ↦ x ^ n`. -/
...
/-- `cfc_comp` specialized to the function `f := fun x ↦ s • x`. -/
...
/-- `cfc_comp` specialized to the function `f := fun x ↦ r * x`. -/
...
/-- `cfc_comp` specialized to the function `f := star`. -/
...
open Polynomial in
/-- `cfc_comp` specialized to polynomials. -/
...
/-- Spectral conditions for `a` to be a scalar multiple of the identity. Note that because
`A` may be the trivial algebra, this isn't an `↔` statement. The converses are supplied below
with a `[Nontrivial A]` hypothesis. -/
...
/-- In the case when `R := ℂ` this would follow from `cfc_predicate`, but this result works
in general. -/
@[simp]
...
variable (R) in
/-- In a nontrivial `R`-algebra with a continuous functional calculus, every element satisfying
the predicate has nonempty `R`-spectrum. -/
...
/-
The following section contains interactions between the continuous functional calculus,
invertible elements, and functions which are nonzero on the spectrum.
-/
section Inv
...
/-- `cfc f a` is invertible if and only `f` is nonzero on the spectrum of `a`. -/
...
By creating a term of type `Aˣ` instead of simply proving `IsUnit (cfc f a)`, we can reason
about integer powers of `cfc f a` more naturally. -/
@[simps]
...
/-- applying the continuous functional calculus to the function `fun x ↦ x⁻¹` yields the inverse
of a unit `a : Aˣ`. -/
...
-- by tagging these with the `fun_prop` attribute, Lean is able to prove continuity automatically
-- in certain situations using the autoparam `cfc_cont_tac`.
@[fun_prop]
...
/-- This is `cfc_comp` specialized to `f := fun x ↦ x⁻¹` when `a` is invertible. -/
...
/-- This is `cfc_comp` specialized to `f := fun x ↦ x ^ n` (with `n : ℤ`) when `a` is invertible. -/
...
/-
Up until this point, all the lemmas have pertained to `R` which is either a semiring or semifield,
so that they apply to `R := ℝ≥0`.

The following section contains properties about negation and subtraction, and so only apply when
`R` is a ring.
-/
...
/-- This is `cfc_comp` specialized to `f := fun x ↦ -x`. -/
...
/-
The next section contains properties about the ordering in the algebra.
Most textbooks on C⋆-algebra theory *define* an element to be nonnegative if it is
selfadjoint and has nonnegative spectrum. It is then a lemma that an element (in a
C⋆-algebra) is nonnegative if and only if it is can be written in the form `star x * x`.

Mathlib takes a different approach. The condition `StarOrderedRing A` says that an
element in `A` is nonnegative if and only if it is (a finite sum of elements) of the form
`star x * x`. We opt for the algebraic definition mostly because it offers the opportunity
to define nonnegativity with fewer prerequisites.
-/
...
Every once in a while, it is necessary to have the full power of the continuous functional calculus
in the form of `cfcHom`. However, sometimes even that is not enough. Consider the following
situation which occurs in practice.

Suppose `a : A` is selfadjoint, then we can write `a = a⁺ - a⁻` with `a⁺ * a⁻ = a⁻ * a⁺ = 0`
and `a⁺` and `a⁻` nonnegative. In fact, these conditions uniquely characterize this decomposition.
That is, if `b` and `c` are nonnegative elements such that `a = b - c` and `b * c = c * b = 0`, then
`b = a⁺` and `c = a⁻`.

How does the proof proceed? Since each of `a`, `b` and `-c` is selfadjoint is has a continuous
functional calculus over `ℝ`. We claim that for every `f : ℝ → ℝ` which is continuous on the union
of the spectra of these three elements, `cfc f a = cfc f b + cfc f (-c)`, and applying this to
`f := fun x ↦ x⁺` shows that `a⁺ = b + 0 = b` (by spectral considerations).-

However, why is the claim that `cfc f a = cfc f b + cfc f (-c)` true? The proof is essentially that
we can interpret each side as a continuous star homomorphism on the union of the spectra which
sends the (restriction of the) identity to `a` (`= b + (-c)`). Then we apply uniqueness of such
homomorphisms.

Unfortunately, realizing both sides of this as a star homomorphism going straight from `cfcHom` is
a somewhat annoying amount work. So instead, we provide `cfcHomSuperset` which is just the
composition of `cfcHom` with the natural embedding of `C(s, R)` into `C(spectrum R a, R)` whenever
`spectrum R a ⊆ s`.

This exists entirely for convenience, and shouldn't be necessary in most cases.
-/

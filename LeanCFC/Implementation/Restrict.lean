...
This file does not contain any of these facts as an `instance`, but they are used in the
`Instances` file to derive the results very easily.
-/
...
This statement is slightly nontrivial because this is a statement about the coercion of these
sets to `Type*`, and these sets are sets in different types: `spectrum S a : Set S`, whereas
`spectrum R a : Set R`.

But this can be used to easily prove that the `ℝ`-spectrum of a selfadjoint element is
homeomorhpic to its `ℂ`-spectrum (under the natural maps `(↑) : ℝ → ℂ` and
`Complex.re : ℂ → ℝ`. -/
...
/-- due to the previous result if `spectrum S a` is compact, so is `spectrum R a`. We need this
because compactness of the spectrum is one of the fields we must provide when constructing an
instance of `ContinuousFunctionalCalculus`. -/
...
This definition only defines the star algebra homomorphism, it does not yet prove that it satisfies
all the extra properties.

This is essentially the composition of the follwing maps:
```
C(spectrum R a, R) ≃ₐ[R] C(spectrum S a, R) -- induced by the homeomorphism defined earlier.
C(spectrum S a, R) →ₐ[R] C(spectrum S a, S) -- induced by postcomposing with the algebra map `R → S`.
C(spectrum S a, S) →ₐ[R] A -- the original continuous functional calculus (restricted so the
                              scalars are `R` instead of `S`.).
```
-/
@[simps!]
...
/-- This lemma is split out from the `SpectrumRestricts.cfc` result below so that we can use it
to prove `cfcHom_eq_restrict` and `cfc_eq_restrict` below. -/
...
/- sometimes it will be useful to work with functional calculi over two different scalar
rings pertaining to the same element. For example, if `0 ≤ a`, it might be useful to know
that `cfc (NNReal.sqrt : ℝ≥0 → ℝ≥0) a = cfc (Real.sqrt : ℝ → ℝ) a`. This may seem very
straightfoward, as the `ℝ≥0`-calculus is obtained from the `ℝ`-calculus via the restriction
process we just completed.

However, recall that `ContinuousFunctionalCalculus` is a *proposition*, not *data*. In other
words, as soon as you construct an instance of `ContinuousFunctionalCalculus`, you cannot
directly access the star algebra homomorphism that it defines. The way we deal with this is
to prove, immediately after the fact, that it coincides with a certain star algebra
homomorphism by uniqueness. This is what the following two lemmas do for the instance obtained
by restriction.
 -/
...
/-- This section is fundamentally the same as the `SpectrumRestricts` section, so we omit
additional comments. -/

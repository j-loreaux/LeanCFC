...
/-- In a Hausdorff topological `𝕜`-algebra (with `RCLike 𝕜`), any star homomorphism from
`C(s, 𝕜)` to `A` (with `s : Set 𝕜`) is uniquely determined by its value on (the restriction
...
/-- A version of the above specific to `ℝ`, to ease type class synthesis. -/
...
/-- A version of the above specific to `ℂ`, to ease type class synthesis. -/
...
/-
The rest of the `UniqueUnital` section is devoted to geting a version of the above for `ℝ≥0`
(still when the algebra is a topological `𝕜`-algebra).

In order to do this, we first need to develop some basic theory about the relationship between
`C(X, ℝ)` and `C(X, ℝ≥0)`. There is a map `ContinuousMap.toNNReal : C(X, ℝ) → C(X, ℝ≥0)` that
sends `f` to `Real.toNNReal ∘ f`, and this map is continuous (in the compact-open topology).
Moreover, there is an equality `f = fun x ↦ (f.toNNReal x : ℝ) - ((-f).toNNReal x : ℝ)`
for all `f : C(X, ℝ)`.

We must take a star homomorphism map `φ : C(X, ℝ≥0) →⋆ₐ[ℝ≥0] A` and extend it to a star
homomorphism `ψ : C(X, ℝ) →⋆ₐ[ℝ] A` (this is `realContinuousMapOfNNReal φ` below). This is
done by defining `ψ f = φ (f.toNNReal) - φ (-f).toNNReal`.

So we first prove a few equalities about the behavior of `ContinuousMap.toNNReal` under
addition, multiplication and scalar multiplication, and use these to show that `ψ` is indeed
a star homomorphism. We then note that `ψ` is obviously continuous if `φ` is.

The final step is to pass from `ψ` to another map `C(((↑) : ℝ≥0 → ℝ) '' X, ℝ) →⋆ₐ[ℝ] A` by
precomposing with the inclusion map `((↑) : ℝ≥0 → ℝ)`, but this takes place inside the proof
of the `ContinuousMap.UniqueHom` instance.
-/
section NNReal
open NNReal
...
/-- `ContinuousMap.toNNReal` is not additive, but by considering also the negative parts, there
...
/-- `ContinuousMap.toNNReal` is not multiplicative, but by considering also the negative parts,
there is a reasonable formula. -/
...
noncomputable def realContinuousMapOfNNReal (φ : C(X, ℝ≥0) →⋆ₐ[ℝ≥0] A) :
  ...
  map_zero' := by simp
  map_mul' f g := by
    /- This proof technique is meant to minimize manual work, and we use this technique
    repeatedly. Take the behavior of `toNNReal` under addition, apply `φ` to both sides,
    and use the fact that `φ` is a homomorphism to push it to the leaves to the
    expresssion. Then move everything to one side. Up to reordering and cancelling terms,
    this exactly proves our goal. -/
...
/-- This shows that a continuous star algebra homomorphism from `C(s, ℝ≥0)`,
(where `s : Set ℝ≥0` is compact) into a topological `ℝ`-alegebra is uniquely
determined by its value on (the restriction of) the identity. -/
...
/- This `UniqueNonUnital` section is very similar to the previous one, so we omit the detailed
comments. -/
...
/- In this section (and the next), we show that (continuous) star homomorphisms commute
with the continuous functional calculus. There makes use of uniqueness in a key way.
However, there is a bit of added subtlety which arises from the following fact: oftentimes
our star algebra homomorphism be over the scalar ring `ℂ`, even though our continuous
functional calculus may be applied to a nonnegative element with a functino `f : ℝ≥0 → ℝ≥0`.

Of course, we could provide specialized proofs for each case, but we want to avoid such
splits whenever possible. Therefore we keep track of both scalar rings in the statement
of this theorem. -/
...
lemma NonUnitalStarAlgHomClass.map_cfcₙ (φ : F) (f : R → R) (a : A)
  ...
  -- first we reinterpret `φ` as a homomorphism over the scalar subring `R`.
  ...
  have hψa : q (ψ a) := hφa
  -- We now bundle the inclusion map `quasispectrum R (ψ a) → quasispectrum R a` as into
  -- a continuous function (sending zero to zero!) acting on the subtypes.
  let ι : C(quasispectrum R (ψ a), quasispectrum R a)₀ :=
    ⟨⟨Set.inclusion h_spec, continuous_id.subtype_map h_spec⟩, rfl⟩
  /- it suffices to show that `φ` (reinterpreted over the scalar ring `R`) composed with the
  continuous functional calculus on `a` is equal to the continuous functional calculus on
  `φ a` composed with the natural ⋆-homomorphism induced by `ι`. -/
  ...
  -- this essentially follows as an application of the uniqueness of the continuous
  -- functional calculus

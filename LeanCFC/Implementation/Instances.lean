...
This file defines instances of the continuous functional calculus for selfadjoint elements
(over `ℝ`) and nonnegative elements (over `ℝ≥0`) that can be obtained from ones over `ℂ` or
`ℝ`, respectively. It also constructs instances of the non-unital continuous functional calculus
for non-unital algebras by pulling back instances of the unital continuous functional calculus
on the unitization of that algebra.

This file remains relatively agnostic about the algebra `A` (i.e., we do not require it to be
a C⋆-algebra). The selfadjoint instance is straightforward. On the other hand, because we are
not working over a C⋆-algebra, and because Mathlib defines nonnegativity in a `StarOrderedRing`
in terms of sums of `star x * x`, the instance for nonnegative elements is trickier. In particular,
we need another class `NonnegSpectrumClass` which connects nonnegativity to a spectral
characterization, namely that `0 ≤ a` implies `∀ x ∈ spectrum ℝ a, 0 ≤ x`.

The unitization argument works over both `ℝ` and `ℂ`, but for the moment, Mathlib only has a
continuous functional calculus instance on `Unitization ℂ A`, because we have no theory of real
C⋆-algebras yet..
...
-- `p` is a predicate on `A`, and `p₁` is the corresponding predicate on `A⁺¹`
-- So, e.g., `p : A → Prop := IsStarNormal` and `p₁ : A⁺¹ → Prop := IsStarNormal`.
-- in that context, the hypothesis `hp₁` just says that `x : A` is normal iff it is normal
-- in `A⁺¹`.
...
calc
  ...
  _             →⋆ₐ[𝕜] A⁺¹ := cfcHom
```
The range of this map is contained in the range of `(↑) : A → A⁺¹` (see `cfcₙAux_mem_range_inr`),
...
/-- By the Stone-Weierstrass theorem, the range of `cfcₙAux` is contained in the image of `A`
within `A⁺¹` under the natural coercion. This is what allows us to pull `cfcₙAux` back to a
function with codomain `A`. -/
...
/-- The is the theorem that obtains a non-unital continuous functional calculus on
`A` from a unital one on `Unitization 𝕜 A`. -/
theorem RCLike.nonUnitalContinuousFunctionalCalculus :
  ...
  exists_cfc_of_predicate a ha := by
    -- restrict the codomain of `cfcₙAux` to `A`
...
/-- This turns `RCLike.nonUnitalContinuousFunctionalCalculus` into an *instance* when the
predicate is `IsSelfAdjoint`. -/
...
/-- Given a continuous functional calculus over `ℝ`, every element whose spectrum is contained in
`ℝ≥0` has a selfadjoint square root. -/
...
/-- In an algebra with a continuous functional calculus over `ℝ` satisfying
`StarOrderedRing`, `NonnegSpectrumClass`, nonnegativity is equivalent to being selfadjoint
and having the (quasi)spectrum be nonnegative. -/
...
open NNReal in
/-- This is the instance of the continuous functional calculus over `ℝ≥0` constructed from
the one for `ℝ`. -/
...
As mentioned in the `Restrict` file, even though we have constructed the continuous functional
calculus on the scalar subrings from the ones on the larger rings, this is only a proposition,
so this fact is immediately forgotten.

In the remaining sections of this file, we recover those facts using the uniqueness of the

...
algebra must *be* a C⋆-algebra (over `ℂ`, or maybe `ℝ`) already. As such, it may seem like this class
is not useful. However, the main purpose is to allow for the continuous functional calculus to be a
isometric for the other scalar rings `ℝ` and `ℝ≥0` too. Later, this will also allow for the
generalization of some results to real C⋆-algebras.

Most of the lemmas in this file are fairly straightforward. They may seem a bit repetitive, but
experience shows that it is often better to have many variations of simple lemmas that receive heavy
use, which we expect will be the case for lemmas in this file.

We feel the statements of these theorems are self-exlanatory and don't require further comment. -/

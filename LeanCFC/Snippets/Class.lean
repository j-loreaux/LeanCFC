...
class ContinuousFunctionalCalculus (R : Type*) {A : Type*} [CStarAlgebra A]
...
open Topology in
class ContinuousFunctionalCalculus (R : Type*) {A : Type*} (p : outParam (A → Type*))
    [CStarAlgebra A] [CommSemiring R] [StarRing R] [MetricSpace R]
    [TopologicalSemiring R] [ContinuousStar R] [Algebra R A] where
